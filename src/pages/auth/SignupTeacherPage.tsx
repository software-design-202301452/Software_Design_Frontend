...
export default function SignupTeacherPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      alert('교사 회원가입이 완료되었습니다. 로그인 해주세요.')
      navigate('/login')
    } catch (err: unknown) {
      const msg =
  ...
  const fields = [
    ...
    { key: 'teacherCode', label: '교사 등록 코드', type: 'text', required: true, placeholder: '관리자에게 받은 코드 입력' },
    { key: 'department', label: '담당 교과', type: 'text', required: false },
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: '#FBF8FA' }}>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold font-display" style={{ color: '#091426' }}>교사 회원가입</h1>
          <p className="text-sm mt-1" style={{ color: '#45474C' }}>교사 계정을 생성합니다</p>
        </div>
